/**
 * Export load
 */

export default load

/**
 * Types
 */

type Options = {
  type?: string
  charset?: string
  async?: boolean
  attrs?: Attributes
  text?: string
}

type Attributes = {
  [key: string]: string
}

type Callback = (err: Error | null, script: HTMLScriptElement) => void

/**
 * Load the script
 *
 * Note: The multiple signatures is typescript's overloading
 */

function load(src: string, opts: Options, cb: Callback): void
function load(src: string, opts: Callback, cb: void): void
function load(src: any, opts: any, cb: any): void {
  var parent = document.body || document.head || document.documentElement
  var script = document.createElement('script')
  if (typeof opts === 'function') {
    cb = opts
    opts = {}
  }

  opts = opts || {}
  cb = cb || function() {}

  script.type = opts.type || 'text/javascript'
  script.charset = opts.charset || 'utf8'
  script.async = 'async' in opts ? !!opts.async : true
  script.src = src

  if (opts.attrs) {
    setAttributes(script, opts.attrs)
  }

  if (opts.text) {
    script.text = '' + opts.text
  }

  var onend = 'onload' in script ? onEnd : ieOnEnd
  onend(script, cb)

  // some good legacy browsers (firefox) fail the 'in' detection above
  // so as a fallback we always set onload
  // old IE will ignore this and new IE will set onload
  if (!script.onload) {
    onEnd(script, cb)
  }

  // append script
  parent.appendChild(script)
}

/**
 * Set the attributes
 */

function setAttributes(script: HTMLScriptElement, attrs: Attributes) {
  for (var attr in attrs) {
    script.setAttribute(attr, attrs[attr])
  }
}

/**
 * onEnd
 */

function onEnd(script: HTMLScriptElement, cb: Callback) {
  script.onload = function() {
    this.onerror = this.onload = null
    cb(null, script)
  }
  script.onerror = function() {
    // this.onload = null here is necessary
    // because even IE9 works not like others
    this.onerror = this.onload = null
    cb(new Error('Failed to load ' + this.src), script)
  }
}

/**
 * onEnd for old IEs
 *
 * TODO: This could probably be removed, I'm not sure which IEs
 * this was relevant for.
 */

function ieOnEnd(script: any, cb: Callback) {
  script.onreadystatechange = function() {
    if (this.readyState != 'complete' && this.readyState != 'loaded') return
    this.onreadystatechange = null
    cb(null, script) // there is no way to catch loading errors in IE8
  }
}
